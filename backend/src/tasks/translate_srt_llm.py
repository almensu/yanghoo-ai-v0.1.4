"""
LLM Subtitle Translation Module

Translates SRT subtitle files from one language to another using LLM APIs.
...
def parse_srt_file(srt_path: str) -> List[Dict]:
    ...
    for block in blocks:
        ...
        try:
            ...
            logger.warning(f"Failed to parse block: {block[:50]}... Error: {e}")
            continue
...
def deduplicate_subtitles(subtitles: List[Dict]) -> List[Dict]:
    """
    Remove duplicate subtitle content (cumulative VTT issue).
...
async def translate_subtitles_with_gemini(
    subtitles: List[Dict],
    ...
    if len(translated_subs) != len(subtitles):
        logger.warning(f"Translation count mismatch: expected {len(subtitles)}, got {len(translated_subs)}")
...
async def translate_srt_file(
    ...
    # Translate
    translated_subs = await translate_subtitles_with_gemini(
        subtitles,
        model=model,
    ...
    logger.info(f"Wrote translated SRT to {srt_output_path}")
    ...
    if txt_output_path:
        ...
        # Write clean text file (paragraphs separated by double newlines)
        clean_text_content = '\n\n'.join(text_lines)

        with open(txt_output_path, 'w', encoding='utf-8') as f:
            f.write(clean_text_content)

        logger.info(f"Wrote clean text to {txt_output_path}")
    ...
    return {
        ...
        "srt_output": srt_output_path,
        "txt_output": txt_output_path,
        "model_used": model
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Translate SRT subtitles using Gemini API',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Translate English SRT to Chinese
  python translate_srt_llm.py input_en.srt output_zh.srt

  # Also generate clean text file
  python translate_srt_llm.py input_en.srt output_zh.srt --txt-output clean_zh.txt

  # Use different model
  python translate_srt_llm.py input_en.srt output_zh.srt --model gemini-2.5-pro

  # Adjust chunk size for API limits
  python translate_srt_llm.py input_en.srt output_zh.srt --chunk-size 30
        """
    )
    ...
    parser.add_argument('--txt-output', help='Output clean text file path (optional)')
    ...
    parser.add_argument('--target-lang', default='Chinese',
                       help='Target language name (default: Chinese)')
    parser.add_argument('--chunk-size', type=int, default=50,
                       help='Number of subtitles per batch (default: 50)')
    ...
    try:
        ...
        result = asyncio.run(translate_srt_file(
            srt_input_path=args.input_srt,
            srt_output_path=args.output_srt,
        ...
        print(f"  Original: {result['original_count']} subtitles")
        print(f"  Translated: {result['translated_count']} subtitles")
        print(f"  Deduplication: {'Enabled' if result['deduplicated'] else 'Disabled'}")
        print(f"  SRT output: {result['srt_output']}")
        if result['txt_output']:
            print(f"  Text output: {result['txt_output']}")
